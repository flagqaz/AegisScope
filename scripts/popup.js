...
const __pageBootHintsPopup = Object.freeze({ cache: 'panel:V2.2.7:7e3d86f67764:99adf0bf9fce', sync: '30d9325267eb756a', mode: 'external-auth-required' });
...
async function init() {
  ...
  els.downloadAll.addEventListener('click', () => {
    runUiActionOnce('save-dialog', () => {
      setActiveActionButton(els.downloadAll);
      openSaveDialog();
    });
  });
  els.siteSniff.addEventListener('click', () => runUiActionOnce('site-sniff', () => openSiteSniff({ active: true })));
...
function runUiActionOnce(key, action, cooldown = 320) {
  if (uiActionLocks.has(key)) return;
  uiActionLocks.add(key);
  let result;
  try {
    result = action();
  } catch (err) {
    setTimeout(() => uiActionLocks.delete(key), cooldown);
    throw err;
  }
...
function ensureSniffExtendedRulesLoaded() {
  if (window.AEGISSCOPE_SNIFF_RULE_LOADER?.load) {
    return window.AEGISSCOPE_SNIFF_RULE_LOADER.load();
  }
...
function scheduleSniffExtendedRulesLoad() {
  if (window.AEGISSCOPE_SNIFF_RULE_LOADER?.schedule) {
    window.AEGISSCOPE_SNIFF_RULE_LOADER.schedule();
  } else {
    setTimeout(() => ensureSniffExtendedRulesLoaded().catch(() => {}), 0);
  }
}
...
function normalizeSniffSignals(page = {}, runtime = {}, background = {}) {
  ...
  return {
    ...
    jqueryRuntime: runtime.jqueryRuntime || [],
    main
  };
}
...
function buildSniffSignalIndex(signals) {
  ...
  return {
    ...
    haystacks: {
      ...
      jqueryRuntime: haystack(signals.jqueryRuntime, 12000),
...
function bestSniffEvidenceVersion(evidences) {
  const priority = ['globals', 'vueRuntime', 'jqueryRuntime', 'meta', 'headers', 'scriptSrc', 'resourceUrl', 'html', 'scripts', 'text'];
  let best = '';
...
function sniffSourceAvailable(signals, source) {
  ...
  if (source === 'jqueryRuntime') return Boolean(signals.jqueryRuntime?.length);
  ...
  for (const item of values) {
    ...
    if (matcher.equals != null) {
      ...
      if (matched) {
        ...
        version = formatSniffMatcherVersion(version, matcher.versionFormat);
      }
    }
    if (matched) {
      out.push({
...
function formatSniffMatcherVersion(version, format) {
  const text = String(version || '').trim();
  if (format === 'jqueryCompact' && /^\d{2,4}$/.test(text)) {
    if (text.length === 2) return `${text[0]}.${text[1]}`;
    if (text.length === 3) return `${text[0]}.${text[1]}.${text[2]}`;
    return `${text[0]}.${text.slice(1, -1)}.${text.slice(-1)}`;
  }
  return text;
}
...
function valuesForSniffSource(signals, matcher) {
  ...
  if (matcher.source === 'jqueryRuntime') {
    return (signals.jqueryRuntime || []).map((value) => ({ source: 'jQuery Runtime', value }));
  }
...
function renderSniffFinding(item) {
  ...
  if (item.name === 'Vue.js') {
    return `
      <div class="sniff-tech-row sniff-tech-row-vue">
        ${main}
  ...
  if (item.name === 'jQuery') {
    const jqueryCandidates = findJqueryScriptCandidates(item);
    const jquerySrc = jqueryCandidates.find((candidate) => candidate.score >= 70)?.url || '';
    return `
...
function findJqueryScriptUrl(item) {
  return findJqueryScriptCandidates(item).find((candidate) => candidate.score >= 70)?.url || '';
}
...
function scoreJqueryCoreUrl(url, targetVersion = '') {
  let file = '';
  let path = '';
  try {
  ...
  if (!/jquery|jq\d{2,4}/i.test(name + path)) return 0;
  if (/(superslide|validate|validation|easing|cookie|form|ui|mobile|mousewheel|colorbox|fancybox|chosen|ztree|datatable|lazyload|template|tmpl|plugin|plugins|migrate|slider|carousel|datepicker|qrcode)/i.test(name)) return 16;
  let score = 0;
  if (/^jquery(?:-\d+(?:\.\d+){0,3})?(?:\.min)?\.js$/i.test(file)) score = 100;
  else if (/^jquery(?:\.\d+(?:\.\d+){0,3})?(?:\.min)?\.js$/i.test(file)) score = 96;
  else if (/^jq(?:uery)?\d{2,4}(?:\.min)?\.js$/i.test(file)) score = 94;
  else if (/^jquery(?:[-_.]min)?\.js$/i.test(file)) score = 92;
  else if (/(?:^|[/_-])jquery(?:[/_-]|$)/i.test(path) && /(?:jquery|jq)\S*\.js$/i.test(file)) score = 76;
  else if (/jquery/i.test(file) && /\d/.test(file)) score = 58;
  else if (/jquery/i.test(path)) score = 34;
  const version = String(targetVersion || '').match(/\d+(?:\.\d+){1,3}/)?.[0] || '';
  if (score && version) {
    const compact = version.replace(/\./g, '');
    if (path.includes(version) || name.includes(version) || name.includes(compact)) score += 18;
  }
...
function sniffConfidenceReason(item) {
  ...
  if (sourceTypes.has('globals') || sourceTypes.has('vueRuntime') || sourceTypes.has('jqueryRuntime')) return '运行时强证据';
...
async function openBeianQuery(options = {}) {
  ...
  hideAssistPanels();
  setAssetListVisible(false);
  if (!options.bypassCache && beianState.result) {
    const currentDomain = normalizeBeianDomain(currentTabHost);
    const cachedDomain = normalizeBeianDomain(
      beianState.result.queryDomain ||
      beianState.signals?.queryDomain ||
      beianState.signals?.rootDomain ||
      beianState.signals?.host ||
      ''
    );
    if (cachedDomain && (!currentDomain || cachedDomain === currentDomain)) {
      els.beianDomain.value = beianState.result.queryDomain || cachedDomain;
      renderBeianResults();
      els.beianStatus.textContent = `已显示缓存：${beianState.result.findings?.length || 0} 条备案线索`;
      setStatus(`备案查询已从缓存显示：${beianState.result.findings?.length || 0} 条线索`);
      return;
    }
  }
...
function sniffEvidenceTitle(item) {
  const fullName = item.version ? `${item.name} ${item.version}` : item.name;
  const evidence = item.evidences.map((ev) => `${ev.source}${ev.key ? ':' + ev.key : ''} = ${ev.value}`).join('\n');
...
function collectSniffPageSignalsInPage(extraSelectors = []) {
  ...
  for (const [selector, marker] of [
    ...
    ['[data-vue-meta]', 'data-vue-meta'],
    ['[data-n-head]', 'data-n-head'],
    ['[data-head-attrs]', 'data-head-attrs'],
    ['#__nuxt, #__layout, [id^="__nuxt"]', 'nuxt-root'],
    ['router-view, router-link', 'router-element'],
    ['[v-cloak], [v-pre], [v-once], [v-if], [v-show], [v-for], [v-model]', 'vue-directive-attr']
  ]) {
...
function collectSniffRuntimeSignalsInPage(extraChains = []) {
  ...
  try {
    ...
    if (Array.isArray(hook?.apps)) {
      for (const app of hook.apps.slice(0, 8)) {
        if (app?.version) pushVueRuntime(`version:${app.version}`);
        if (app?.app?._context || app?.app?.config) pushVueRuntime('devtools-app-context');
      }
    }
  } catch {}
  try {
    if (window.Vue?.version) pushVueRuntime(`version:${window.Vue.version}`);
    if (window.__VUE__) pushVueRuntime('__VUE__');
    if (window.__VUE_HOT_MAP__) pushVueRuntime('__VUE_HOT_MAP__');
    if (window.$nuxt || window.__NUXT__ || window.useNuxtApp) pushVueRuntime('nuxt-runtime');
    if (window.VueRouter) pushVueRuntime('vue-router-runtime');
    if (window.Vuex) pushVueRuntime('vuex-runtime');
  ...
  try {
    for (const node of document.querySelectorAll('*')) {
      ...
      if (node.__vue__) {
        pushVueRuntime('__vue__');
        const version = node.__vue__?.$root?.constructor?.version || node.__vue__?.constructor?.version;
        if (version) pushVueRuntime(`version:${version}`);
      }
      if (node.__vueParentComponent) {
        pushVueRuntime('__vueParentComponent');
        const version = node.__vueParentComponent?.appContext?.app?.version;
        if (version) pushVueRuntime(`version:${version}`);
      }
      if (node.__vnode || node._vnode) {
        pushVueRuntime('__vnode');
  ...
  const jqueryRuntime = [];
  const pushJqueryRuntime = (value) => {
    if (value && !jqueryRuntime.includes(value)) jqueryRuntime.push(value);
  };
  try {
    const jq = window.jQuery;
    const dollar = window.$;
    const jqVersion = jq?.fn?.jquery || jq?.prototype?.jquery || '';
    const dollarVersion = dollar?.fn?.jquery || dollar?.prototype?.jquery || '';
    if (jqVersion) pushJqueryRuntime(`version:${jqVersion}`);
    if (dollarVersion && dollarVersion !== jqVersion) pushJqueryRuntime(`version:${dollarVersion}`);
    if (jq?.fn) pushJqueryRuntime('jQuery.fn');
    if (dollar?.fn?.jquery) pushJqueryRuntime('dollar.fn.jquery');
    if (jq?.migrateVersion) pushJqueryRuntime(`migrate:${jq.migrateVersion}`);
    if (jq?.ui?.version) pushJqueryRuntime(`ui:${jq.ui.version}`);
    if (jq?.mobile?.version) pushJqueryRuntime(`mobile:${jq.mobile.version}`);
    if (jq?.fn?.select2) pushJqueryRuntime('plugin:select2');
...
async function openSecurityScan() {
  const url = chrome.runtime.getURL(`pages/scan.html?tabId=${currentTabId}`);
  await chrome.tabs.create({ url });
}

async function openFingerprintScan() {
  const url = chrome.runtime.getURL(`pages/fingerprint-scan.html?tabId=${currentTabId}`);
  await chrome.tabs.create({ url });
}
...
async function openVulnScan() {
  const url = chrome.runtime.getURL(`pages/vuln-scan.html?tabId=${currentTabId}`);
  await chrome.tabs.create({ url });
}

async function openVueTools() {
  const url = chrome.runtime.getURL(`pages/vue-tools.html?tabId=${currentTabId}`);
  await chrome.tabs.create({ url });
}

async function openJqueryVulnCheck(version = '', jquerySrc = '', candidates = []) {
