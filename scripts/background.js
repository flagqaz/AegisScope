...
const __pageBootHintsBackground = Object.freeze({ cache: 'panel:V2.2.6:7e3d86f67764:99adf0bf9fce', sync: '30d9325267eb756a', mode: 'external-auth-required' });
...
async function ensureCopyUnlockPreloadRegistered() {
  ...
  try {
    await chrome.scripting.registerContentScripts([{
      ...
      js: ['scripts/injected/copy-unlock.js'],
...
async function ensureWebrtcPreloadRegistered(strongBlock) {
  ...
  try {
    await chrome.scripting.registerContentScripts([{
      ...
      js: ['scripts/injected/webrtc-guard.js'],
      runAt: 'document_start',
      allFrames: true,
      world: 'MAIN'
    }]);
...
async function injectWebrtcGuard(tabId) {
  ...
  try {
    const results = await chrome.scripting.executeScript({
      ...
      files: ['scripts/injected/webrtc-guard.js'],
      world: 'MAIN',
...
function uaProfiles() {
  return globalThis.AEGISSCOPE_UA_FALLBACK_PROFILES || [];
}
...
async function ensureUaScripts(config) {
  ...
  try {
    await chrome.scripting.registerContentScripts([{ id: UA_MAIN_SCRIPT_ID, js: ['scripts/injected/ua-page.js'], world: 'MAIN', ...common }]);
    await chrome.scripting.registerContentScripts([{ id: UA_ISOLATED_SCRIPT_ID, js: ['scripts/injected/ua-isolated.js'], world: 'ISOLATED', ...common }]);
    return true;
  } catch {
    return false;
  }
...
async function injectUa(tabId, config) {
  ...
  const payload = parseUaPayload(selectedUa(config), config.exposeUserAgentData);
  try {
    await chrome.scripting.executeScript({ target: { tabId, allFrames: true }, files: ['scripts/injected/ua-page.js'], world: 'MAIN', injectImmediately: true });
    const results = await chrome.scripting.executeScript({
      target: { tabId, allFrames: true },
      world: 'MAIN',
      injectImmediately: true,
