...
async function startScan() {
  ...
  try {
    ...
    state.findings = resolveFindings(analyzeResponses(state.responses));
    render();
    ...
    for (let i = 0; i < urls.length; i++) {
      ...
      if (item) state.responses.push(item);
      state.findings = resolveFindings(analyzeResponses(state.responses));
      render();
    }
    if (!state.stopped) {
...
function buildScanUrls(observed) {
  ...
  for (const url of uniqueList(probeUrls).slice(0, 30)) add(url);
  const resourceUrls = [];
  ...
  for (const url of uniqueList(resourceUrls).slice(0, 34)) add(url);
  return Array.from(urls).slice(0, 76);
}
...
async function fetchFingerprintUrl(url) {
  ...
  try {
    ...
    const body = isText ? decodeBytes(bytes, type).slice(0, 1100000) : '';
    const headers = {};
  ...
  for (const rule of rules) {
    ...
    for (const response of responses) {
      for (const matcher of rule.matchers || []) {
        ...
        if (!version && evidence.version) version = evidence.version;
      }
    }
    if (!evidences.length) continue;
    const uniq = uniqueEvidence(evidences);
    ...
    hits.push({
      ...
      score,
      version: version || inferVersion(rule, uniq),
...
function matchRule(response, matcher) {
  ...
  for (const item of values) {
    ...
    let excerpt = '';
    if (matcher.hash) {
      matched = matcher.hash.map(String).includes(value);
      excerpt = value;
    } else if (matcher.all) {
      matched = matcher.all.every((part) => value.toLowerCase().includes(String(part).toLowerCase()));
      ...
      const match = safeRegex(matcher.regex).exec(value);
      matched = !!match;
      excerpt = match?.[0] || '';
...
function valuesForMatcher(response, matcher) {
  if (matcher.source === 'body') return canUseBodyForFingerprint(response, matcher) ? [{ source: 'Body', value: response.body || '' }] : [];
  if (matcher.source === 'title') return canUseTitleForFingerprint(response, matcher) ? [{ source: 'Title', value: response.title || '' }] : [];
  if (matcher.source === 'url') return [{ source: 'URL', value: `${response.url}\n${response.requestedUrl}` }];
  if (matcher.source === 'script') return canUseScriptForFingerprint(response, matcher) ? [{ source: 'Script/URL', value: `${response.url}\n${response.body || ''}` }] : [];
  if (matcher.source === 'favicon') return [{ source: 'FaviconHash', value: response.faviconHash || '' }];
  if (matcher.source === 'header') {
    const key = String(matcher.key || '').toLowerCase();
    if (!key) return Object.entries(response.headers || {}).map(([name, values]) => ({ source: 'Header', key: name, value: values.join('\n') }));
    return (response.headers?.[key] || []).map((value) => ({ source: 'Header', key, value }));
  }
  return [{ source: 'Body', value: response.body || '' }];
}
