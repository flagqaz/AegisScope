...
![玄镜 AegisScope 界面预览](docs/images/main-popup-overview.png)
...
- 调整主流程、工具区、网站嗅探、备案查询等区域的显示层级，减少页面杂乱感。
- 优化网站嗅探结果展示，三列布局更规整，长名称省略后悬停可查看完整名称和命中证据。
- 简化备案查询展示，隐藏顶部查询说明区，让备案线索和接口结果更靠前。
- 增加网站嗅探和备案查询缓存复用，重复切换同一目标时优先显示已有结果；刷新全部仍会强制重新查询。
- 增加轻量防重复点击处理，减少连续切换功能时的卡顿和重复任务。
### V2.2.6
